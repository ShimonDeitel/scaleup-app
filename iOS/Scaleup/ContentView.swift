...
struct EntryFormView: View {
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Weightkg", value: $draftWeightKg, format: .number)
                        .accessibilityIdentifier("field_weightKg")
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = WeightEntry(date: draftDate, weightKg: draftWeightKg, notes: draftNotes)
            store.add(entry)
        case .edit(var entry):
            entry.date = draftDate
            entry.weightKg = draftWeightKg
